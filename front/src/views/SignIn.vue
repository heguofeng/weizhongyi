  ...
  methods: {
    ...
    signIn: function(){
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.logining2 = true;
          this.axios.post('/api/admin', { "token": this.token, "security_code": this.form.security_code }).then((response)=> {
            this.logining2 = false;
            if (response.data.success) {
              var id = response.data.result; //得到新建记录的id
              this.form.token = this.token;//把token放进form对象里传递
              this.axios.put('/api/admin/' + id, this.form).then((response)=> {          
                this.$message.success("已成功注册账号： " + response.data.result.phone);
              },(err) =>{
